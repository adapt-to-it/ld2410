...
> **Compared to the LD2410.** The LD2420 shares the same wire framing as the LD2410 family (`FD FC FB FA` … `04 03 02 01`, little-endian, 2-byte intra-frame length) but exposes a **register-level / ABD parameter** command vocabulary instead of the LD2410's high-level configuration commands. There is no built-in human-presence classifier in the protocol layer — the LD2420 reports raw FFT energies and you implement the policy on the host. The data-output frame format (energy report frames; on-wire header `F4 F3 F2 F1` / footer `F8 F7 F6 F5` — same envelope bytes as the LD2410 family) is documented separately in [`HLK-LD2420_data_format.md`](HLK-LD2420_data_format.md), cross-checked against ESPHome's production `ld2420` component. This file covers only the **command** side, as the XLSX does.
