...
| `0x0001` | §1.2.2 | Write register | ❌ | _not exposed yet_ — payload is `chip_addr (2B) + (reg_addr (2B) + reg_data (2B)) × N`. Planned: `writeRegister(uint16_t chip, uint16_t reg, uint16_t value)` + a bulk variant | `LD2420_HAS_REGISTER_RW` |
| `0x0002` | §1.2.3 | Read register | ❌ | _not exposed yet_ — request is `chip_addr (2B) + reg_addr (2B) × N`. Planned: `readRegister(uint16_t chip, uint16_t reg, uint16_t & out)` | `LD2420_HAS_REGISTER_RW` |
...
| `0x0012` | §1.2.7 | Configure system parameters | ❌ | _not exposed yet_ — payload is `(param_word (2B) + value (4B)) × N`; param words enumerated in `LD2420_SYS_W_*`. Most useful: `LD2420_SYS_W_MODE` switching between transparent / MTT / VS / GR / custom | `LD2420_HAS_SYSTEM_PARAMS` |
| `0x0013` | §1.2.8 | Read system parameters | ❌ | _not exposed yet_ | `LD2420_HAS_SYSTEM_PARAMS` |
...
1. **`requestRegister` / `writeRegister`** (0x0001 / 0x0002) — gates direct
   access to the radar's chip registers, which the rest of the protocol
   builds on.
2. **`requestAbdParameter` / `setAbdParameter`** (0x0008 / 0x0007) — drives
   the on-chip ABD detection policy. Required for any non-transparent
   `systemMode`.
3. **`requestSystemParameter` / `setSystemParameter`** (0x0013 / 0x0012) —
   in particular `LD2420_SYS_W_MODE` to switch between transparent / MTT /
   VS / GR.
4. **`parse_data_frame_()` real implementation** — decode the per-gate FFT
   energies into a `uint16_t energies[LD2420_GATE_COUNT]` snapshot field +
   atomic snapshot getter.
5. **`requestSerialNumber()`** (0x0011) — quick win once register r/w is in.
6. **Factory-test commands** (0x0024 / 0x0025 / 0x0026) — niche but
...
| 6 | Register r/w (0x0001 / 0x0002) | ⏳ next PR |
| 7 | ABD params (0x0007 / 0x0008) — typed setters + per-gate threshold packing | ⏳ |
| 8 | System params (0x0012 / 0x0013) | ⏳ |
| 9 | `parse_data_frame_()` real FFT-energy decode | ⏳ blocked on transcribing data-frame layout from product-manual PDF |
| 10 | Serial number (0x0011) | ⏳ |
| 11 | Factory-test mode (0x0024 / 0x0025 / 0x0026) | ⏳ |
| 12 | Hardware validation against a physical LD2420 sample | ⏳ blocked on bench acquisition |
| 13 | Host parser unit test (`tests/test_parser_ld2420/`) mirroring `tests/test_parser*/` for LD2410 | ⏳ |
